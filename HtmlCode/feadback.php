...
if ($conn->connect_error) {
    ...
    $sql = "select * from helppage order by fdate desc;";
    $result = $conn->query($sql);
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        $date = $_POST['date'];
        if ($date != '')
            $sql = "select * from helppage where fdate='$date' order by fdate desc;";
        $result = $conn->query($sql);
    }
}
...
<body>
    ...
    <section>
        <h1>Help & Feadback Details</h1>
        <form action="" method="POST" id="fr">
            <p>
                Enter Date In Which Help & Feedback You Want To Show :
